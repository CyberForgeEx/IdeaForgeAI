...
# Comments Management
def add_comment(idea_id, user_id, comment_text):
    """Add a comment to an idea."""
    conn = get_db_connection()
    cursor = conn.execute(
        'INSERT INTO idea_comments (idea_id, user_id, comment) VALUES (?, ?, ?)',
        (idea_id, user_id, comment_text)
    )
...
def delete_comment(comment_id, user_id):
    """Delete a comment (with user ownership check)."""
    conn = get_db_connection()
    cursor = conn.execute(
        'DELETE FROM idea_comments WHERE id = ? AND user_id = ?',
        (comment_id, user_id)
    )
    deleted = cursor.rowcount > 0
    conn.commit()
...
def add_comment_route(idea_id):
    ...
    add_comment(idea_id, session['user_id'], comment_text)
...
def delete_comment_route(comment_id):
    """Delete a comment."""
    if delete_comment(comment_id, session['user_id']):
        flash('Comment deleted successfully.', 'success')
    else:
        flash('Comment not found or unable to delete.', 'error')

    # Redirect back to the idea
